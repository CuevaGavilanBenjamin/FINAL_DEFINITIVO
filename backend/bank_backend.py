...
def handle_sftp_client(client_socket, address):
    ...
    try:
        ...
        transport.set_subsystem_handler('sftp', paramiko.SFTPServer, sftp_si=BankSFTPServer)
        
        server = BankSSHServer()
        
        # Iniciar servidor (pasar server directamente como parámetro)
        ...
        logger.info(f"[SFTP] Channel established with {address}")
        ...
        import traceback
        logger.error(f"[SFTP] Traceback: {traceback.format_exc()}")
    finally:
        try:
            transport.close()
        except:
...
def start_sftp_server(host='0.0.0.0', port=2222):
...
if __name__ == "__main__":
    ...
    # 1. Iniciar servidor SFTP PRIMERO
    start_sftp_server(port=2222)
    logger.info("[BANK] SFTP Server started on port 2222")
    
    # 2. Dar tiempo para que el SFTP se inicie
    time.sleep(2)
    
    # 3. Iniciar API REST (esto bloquea el thread principal)
    uvicorn.run(
        app,
        host="0.0.0.0",
        port=8000,
